...
不需要整套安装：每次只选择当前任务需要的一个 Plugin，并在目标运行端显式安装。先克隆本仓，然后把仓库根目录作为本地 Marketplace 添加；下面的 `<repo-root>` 是本仓的绝对路径，首次示例使用 `grilling`。

源码存在或来源测试通过，只说明仓库中的资产可发现且自洽，不代表 Codex／Claude 运行端已经安装。
